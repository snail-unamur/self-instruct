...
def post_process_api_response(response):
    raw_instructions = re.split(r"\n\d+\s?\. ", response["choices"][0]["generated_text"])
    instructions = []
    for inst in raw_instructions:
        ...
        if len(inst.split()) <= 20 or len(inst.split()) > 500:
            continue
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="The number of requests to send to API at a time."
    )
    parser.add_argument(
        "--api_key",
        type=str,
    ...
    with open(os.path.join(args.batch_dir, "machine_generated_instructions.jsonl"), "a") as fout:
        while len(machine_instructions) < args.num_instructions_to_generate:
            ...
            for result in results:
                new_instructions = post_process_api_response(result["response"])
